...
_Dubs_Composer::_Dubs_Composer() :
        ...
        _suit3(0)
{
    //Assuming dubs composer begins with reset generators
    for (int i=0; i < cards::COMBO_DUBS - 1; i++)
        this->_generate_double();

    //pair starts up on the previous combination of doubles of the 1st
...
void _Dubs_Composer::_next_rank()
{
    ...
    if (this->_shift4 < cards::TWO - 1) {
        ...
        if (this->_shift3 < cards::THREE - 1 || 
            (this->_shift3 == cards::THREE - 1 &&
            (this->_shift1 < cards::FOUR - 1))) {
            decrement_past(this->_shift2, this->_shift1);
            if (this->_shift2 < cards::FOUR - 1 || 
                (this->_shift2 == cards::FOUR - 1 &&
                (this->_shift1 < cards::FIVE - 1))) {
                this->_shift1--;
...
void _Dubs_Composer::_check_finished()
{
    ...
    if (this->_is_finished) _Dubs_Composer::_reset();
}
